...
A production-ready speech-to-text transcription service API powered by OpenAI's Whisper Large V3 Turbo model, deployed on AWS SageMaker.

## Features

- **Accurate Transcription**: Powered by Whisper Large V3 Turbo
- **Multiple Audio Formats**: Supports MP3, WAV, M4A, FLAC, OGG, WebM
- **Fast Processing**: GPU-accelerated transcription on SageMaker
...
```
Internet → ALB → API (Go) → SageMaker → S3
```

- **VPC**: Multi-AZ with public/private subnets
- **ECS Fargate**: API service (Go)
- **SageMaker**: ml.g5.xlarge with Whisper model
...
Primary costs: SageMaker ml.g5.xlarge ($0.736/hour), NAT Gateways, ECS Fargate
