...
This project implements a production-ready speech-to-text transcription service using OpenAI's Whisper Large V3 Turbo model deployed on AWS SageMaker, with a Go API backend.
...
**Instance Type**: ml.g5.xlarge (1x NVIDIA T4 GPU, 4 vCPUs, 16GB RAM)
...
1. Client sends POST request to `/transcribe` endpoint via ALB
2. ALB routes to API Service (ECS)
3. API Service validates file format and size
4. API Service invokes SageMaker endpoint with audio data
5. SageMaker downloads model from S3 (if cold start)
6. SageMaker runs inference with Whisper model
7. SageMaker returns transcription result
8. API Service formats and returns response to client
...
- **SageMaker ml.g5.xlarge**: $0.736/hour × 730 hours = ~$537/month
- **ECS Fargate**:
