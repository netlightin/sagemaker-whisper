...
func (h *Handler) Test(w http.ResponseWriter, r *http.Request) {
	...
	// Parse query parameters for test configuration
	nrOfRequests := 1
	audioDuration := 20

	// Parse required parameters from query string
	if nr := r.URL.Query().Get("nr_of_requests"); nr != "" {
		fmt.Sscanf(nr, "%d", &nrOfRequests)
	}
	if ad := r.URL.Query().Get("audio_duration"); ad != "" {
		fmt.Sscanf(ad, "%d", &audioDuration)
	}

	// Construct audio file path
	audioFile := fmt.Sprintf("audio/b%d.mp3", audioDuration)

	// Start the load test asynchronously
	testID, err := h.StartLoadTest(audioFile, nrOfRequests)
	if err != nil {
		h.logger.Error("Failed to start load test:", err)
		h.sendError(w, "Failed to start load test", http.StatusInternalServerError)
		return
	}
...
func (h *Handler) TestStatus(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet && r.Method != http.MethodPost {
		h.sendError(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	if testID == "" {
		h.sendError(w, "Test ID (id parameter) required", http.StatusBadRequest)
		return
	}

	status := h.GetTestStatus(testID)
