...
def download_model(model_name, output_dir):
    ...
    logger.info("\n" + "=" * 70)
    logger.info("DOWNLOADING MODEL - THIS WILL TAKE 10-30 MINUTES")
    logger.info("=" * 70)
...
def create_sagemaker_package(model_dir, ml_model_dir, output_file):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        inference_py = ml_model_dir / "loud_meadow" / "inference.py"
...
def upload_to_s3(local_file, s3_bucket, s3_prefix="kb-whisper"):
...
def main():
    ...
    parser.add_argument(
        "--s3-bucket",
        required=True,
        help="S3 bucket name for model artifacts (e.g., sagemaker-loud-meadow-models-123456789-eu-west-1)",
    )
    parser.add_argument(
        "--model-name",
    ...
    parser.add_argument(
        "--output-dir", default="./loud_meadow", help="Local directory to save model"
    )
    parser.add_argument(
        "--package-file",
        default="kb-whisper-model.tar.gz",
        help="Output tar.gz filename",
    )
    parser.add_argument(
        "--skip-upload", action="store_true", help="Skip S3 upload (for testing)"
    )
    ...
    try:
        ...
        package_file = create_sagemaker_package(
            model_dir, ml_model_dir, args.package_file
        )
        ...
        if not args.skip_upload:
            ...
            logger.info("\nTo upload manually:")
            logger.info(
                f"  aws s3 cp {package_file} s3://{args.s3_bucket}/kb-whisper/model.tar.gz"
            )
