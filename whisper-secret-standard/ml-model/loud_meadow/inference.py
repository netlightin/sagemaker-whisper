"""
SageMaker inference script for KB-Whisper Large.
...
def predict_fn(
    ...
    """
    Run inference on the input data with support for long audio by chunking.
    ...
    try:
        ...
        # We need to chunk longer audio and transcribe each chunk
        ...
        logger.info(
            f"Audio will be processed in {num_chunks} chunk(s) ({chunk_duration}s each)"
        )
        ...
        for chunk_idx in range(num_chunks):
            ...
            logger.info(
                f"Processing chunk {chunk_idx + 1}/{num_chunks} ({chunk_duration_actual:.2f}s)"
            )
            ...
            # Decode this chunk's transcription
            chunk_transcription = processor.batch_decode(
                predicted_ids, skip_special_tokens=True
            )[0]

            logger.info(
                f"  Chunk {chunk_idx + 1} transcription length: {len(chunk_transcription)} characters"
            )
