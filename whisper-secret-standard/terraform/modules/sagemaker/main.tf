...
# SageMaker Model
# Note: Model name includes a hash of the image URI to force recreation when image changes
resource "aws_sagemaker_model" "whisper" {
  name               = "${var.project_name}-model-${substr(md5(var.inference_image_uri), 0, 8)}"
...
# Note: Config name includes a hash of the image URI to force recreation when image changes
resource "aws_sagemaker_endpoint_configuration" "whisper" {
  name = "${var.project_name}-endpoint-config-${substr(md5(var.inference_image_uri), 0, 8)}"
